...
%
% Usage:
% This script recapitulates the simulation results for Fig 10 in the above paper.
...
% - For a repetitive train at a given stimulation frequency, an additional
%   test stimulus is inserted into the stim pattern at different locations.
%   The discriminability between test and baseline stimulation results is
%   examined from single cell and population responses.
%
...
for iGenotype = 1:length(GenotypeList)
    ...
    if ispc
        filePath = sprintf('C:\\AD_data\\ModellingResults\\%s_%dHz_AddedDistractor_%s',Genotype,stimFrequency, datestr(datetime('now'), 'mm-dd-yyyy HH-MM-SS'));
    else ismac
        filePath = sprintf('/Users/aleksanderdomanski/Documents/AD_DATA/ModellingResults/%s_%dHz_AddedDistractor_%s',Genotype,stimFrequency, datestr(datetime('now'), 'mm-dd-yyyy HH-MM-SS'));
    end
    mkdir(filePath)
    %% precalculate models
    switch Genotype
    ...
    parfor model_id = 1:no_models
        ...
        tic
        for iStim=1:length(stimPosition)
            
            stimPattern = [5,  stimFrequency, stimPosition(iStim)];
            % 2nd last arguement is "mode": 1 - add a spike, 2 - remove a spike
            M    = L4sim_MakeDistractorInput(Marray{model_id},1000,0.5,[1 1],stimPattern,1,0);
            ...
            for trial_id=1:no_trials
                ...
                % Shuffle cells receiving TC stim
                r=randperm(M.p.Ne)';
                M.net.S_ext(1:M.p.Ne)=M.net.S_ext(r);
                M.net.S_ext_NMDA(1:M.p.Ne)=M.net.S_ext_NMDA(r);
                
                fprintf('Running %s model no. %d, stim distractor pattern %d %dx%dHz, trial no. %d...\n',...
                    Genotype,model_id,stimPattern(3),stimPattern(1),stimPattern(2),trial_id)
                ...
                Fname = sprintf('%s_addedDistractor_%d_%d_%d.mat',Genotype,model_id,stimPosition(iStim),trial_id)
                %             save([filePath filesep  Fname],'M','results','no_models','model_id','stimPosition','iStim','no_trials','trial_id')
                ...
                save_parfor([filePath filesep  Fname],stimPosition)
