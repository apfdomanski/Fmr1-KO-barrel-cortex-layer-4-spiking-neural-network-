function M=L4sim_MakeDistractorInput(M,t_max,dt,Amp,StimPattern,mode,plot_online)

% External input builder with distractor insertion and simulation design for L4 thalamocortical integration spiking model to accompany: Domanski,Booker,Wyllie,Isaac,Kind (2018)
%
% Reference: 
...
% mode                      - 1 - add a spike, 2 - remove a spike
...
% An additional test stimulus is inserted into the stim pattern at different 
% locations. to examine the discriminability between test and baseline 
% stimulation results.
%
...
M.p.t_max = t_max;
M.p.dt    = dt;                           % Simulation timebase (ms)
...
DistractorTime = StimPattern(3);
switch mode
    case 1    % Add
        if DistractorTime>0
            noStims = noStims+1;
            M.input.ZAP.stim_times = sort([M.input.ZAP.stim_times, M.input.ZAP.stim_times(DistractorTime)+ISI/2]);
        end
    case 2    % Subtract
        if DistractorTime>0
            noStims = noStims-1;
            M.input.ZAP.stim_times(DistractorTime) = [];
        end        
end

% Alternative hard coded stim times:
% M.input.ZAP.stim_times(end+1) = 200;
% M.input.ZAP.stim_times = [100 120 140 160 180];
